...
class TreePanelController {
    ...
    static createNewNodeGroupViewingElementVektor(currentInsertedNode, widthOfPanel, heightOfPanel, verticalSpacing, angle, heightOfFirstNode = 5, coeff_0 = 0.6, coeff_1 = 0.8, coeff_else = 1) {
        let angleHere = angle;
        ...
        let horizontalSpacing = verticalSpacing / (Math.tan(angleLocal * Math.PI/180));
        ...
        if(lengthOfPath === 0){
            ...
            for (let index = 0; index < lengthOfPath; index++) {
                ...
                if(index == 0) {
                    ...
                    horizontalSpacing = verticalSpacing / (Math.tan(angleLocal * Math.PI/180));
                }
                else if(index == 1) {
    ...
    static drawArrowForTernaryTree(panelKnowledge_2, allNodes, allNodeGroupElements, radius) {
        let arrayOfXY = [];
        ...
        majorLines[3].setStartPoint(new jsgl.Vector2D(arrayOfXY[0][0], (arrayOfXY[0][1]  + radius) ));
        majorLines[3].setEndPoint(new jsgl.Vector2D(arrayOfXY[3][0], (arrayOfXY[3][1]  - radius) ));

        majorLines[0].setStartPoint(new jsgl.Vector2D(arrayOfXY[0][0], (arrayOfXY[0][1]  + radius) ));
        majorLines[0].setEndPoint(new jsgl.Vector2D(arrayOfXY[3][0], (arrayOfXY[3][1]  - radius) ));

        TreePanelController.drawArrow(panelKnowledge_2, allNodes[2], allNodes[0], radius);
        TreePanelController.drawArrow(panelKnowledge_2, allNodes[2], allNodes[5], radius);

        TreePanelController.drawArrow(panelKnowledge_2, allNodes[4], allNodes[1], radius);
        TreePanelController.drawArrow(panelKnowledge_2, allNodes[4], allNodes[7], radius);

        TreePanelController.drawArrow(panelKnowledge_2, allNodes[0], allNodes[8], radius);
        TreePanelController.drawArrow(panelKnowledge_2, allNodes[0], allNodes[6], radius);

        TreePanelController.drawArrow(panelKnowledge_2, allNodes[0], allNodes[3], radius);
        TreePanelController.drawArrow(panelKnowledge_2, allNodes[5], allNodes[4], radius);

        majorLines[4].setStartPoint(new jsgl.Vector2D(arrayOfXY[5][0], (arrayOfXY[5][1]  + radius) ));
        majorLines[4].setEndPoint(new jsgl.Vector2D(arrayOfXY[4][0], (arrayOfXY[4][1]  - radius) ));
    ...
    static drawArrow(panel, nodeFrom, nodeTo, radius ) {
        let wholeVectorBetweenTwoNodesLength = Math.sqrt(Math.pow((nodeFrom.vector.getX() - nodeTo.vector.getX()), 2) + Math.pow((nodeFrom.vector.getY() - nodeTo.vector.getY()), 2));
        let gegenKatheteVectorBetweenTwoNodes = Math.abs((nodeFrom.vector.getX() - nodeTo.vector.getX()));
        
        let alphaVectorBetweenTwoNodesInRadian = gegenKatheteVectorBetweenTwoNodes / wholeVectorBetweenTwoNodesLength;
        let betaInRadian = Math.PI - 90 * Math.PI/180 - Math.asin(alphaVectorBetweenTwoNodesInRadian);

        let smallGegenKatheteAlpha = radius * Math.sin(alphaVectorBetweenTwoNodesInRadian);
        let smallAnKatheteAlpha = radius * Math.cos(alphaVectorBetweenTwoNodesInRadian);
        ...
        if(nodeFrom.left == nodeTo) {
            ...
            let xOfSmallAnKatheteXFrom = nodeFrom.vector.getX() - smallGegenKatheteAlpha;
            ...
            let leftNodeFromVector = new jsgl.Vector2D(xOfSmallAnKatheteXFrom, yOfSmallAnKatheteYFrom);
            let leftNodeToVector = new jsgl.Vector2D(xOfSmallAnKatheteXTo, yOfSmallAnKatheteYTo);
            ...
            let yOfSmallAnKatheteYFrom = smallAnKatheteAlpha + nodeFrom.vector.getY();
            let xOfSmallAnKatheteXFrom = nodeFrom.vector.getX() + smallGegenKatheteAlpha;
            let rightNodeFromVector = new jsgl.Vector2D(xOfSmallAnKatheteXFrom, yOfSmallAnKatheteYFrom);

            let yOfSmallAnKatheteYTo = nodeTo.vector.getY() - smallAnKatheteAlpha;
            let xOfSmallAnKatheteXTo = nodeTo.vector.getX() - smallGegenKatheteAlpha;
            let rightNodeToVector = new jsgl.Vector2D(xOfSmallAnKatheteXTo, yOfSmallAnKatheteYTo);
            ...
            // majorLine.setZIndex(25);
            // upperMinorLine.setZIndex(25);
            // lowerMinorLine.setZIndex(25);

            panel.addElement(upperMinorLine);
            panel.addElement(lowerMinorLine);
        }
        else if((nodeFrom.left == null && nodeFrom.right == null) || nodeTo.parent != null) {

            // fuer den Fall des ternaeren Baums
            let yOfSmallAnKatheteYFrom = smallAnKatheteAlpha + nodeFrom.vector.getY();
            let xOfSmallAnKatheteXFrom = nodeFrom.vector.getX() + smallGegenKatheteAlpha;
            let rightNodeFromVector = new jsgl.Vector2D(xOfSmallAnKatheteXFrom, yOfSmallAnKatheteYFrom);
    ...
    static designOfSelectedNode(element, radius = 30){
        ...
        element.getStroke().setColor(hellBrownColor);
    ...
    static designEllipse(element) {
        ...
        element.getFill().setColor(hellBrownColor);
    }
}
