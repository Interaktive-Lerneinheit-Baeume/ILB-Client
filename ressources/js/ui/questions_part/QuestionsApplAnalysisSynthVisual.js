...
let panelsTrees = [],
    ...
    valueArrOfArrays = [[6, 5, 4, 3, 2, 1], [4, 6, 5, 3, 1, 2], [1, 2, 3, 4, 5, 6]],
    printTreePanel, printCodePanel,
...
function drawPrintTreeAndCode() {
    ...
    for (let index = 0; index < valueArrayData.length; index++) {
        ...
        let newNodeElement = TreePanelController.createNewNodeElement(element, newTree);
        nodeElementsArray.push(newNodeElement);
        let newNodeGroupViewingElement = TreePanelController.createNewNodeGroupViewingElement(printTreePanel, newNodeElement, index, radius);

        let newNodeGroupViewingElementVektor =
            TreePanelController.createNewNodeGroupViewingElementVektor(newNodeElement, widthPanel, heightPanel, vertSpaceLocal, angleLocal, 6, 0.5, 0.75, 1);
    ...
    for (let index = 0; index < nodeElementsArray.length; index++) {
        ...
        if (element.left != null && element.right != null) {
            ...
            TreePanelController.drawArrow(printTreePanel, element, leftTempNode, radius);
            TreePanelController.drawArrow(printTreePanel, element, rightTempNode, radius);
        }
            ...
            console.log("Stimmt etwas nicht richiutg!");
        }
    }

    CodeVisualizer.drawCode(printCodePanel, labelTextSize);
}
...
function drawThreeTrees() {
    ...
    for (let i = 0; i < valueArrOfArrays.length; i++) {
        ...
        for (let index = 0; index < someArray.length; index++) {
            ...
            let newNodeElement = TreePanelController.createNewNodeElement(someArray[index], newTree);
            outerArrayOfNodes[i].push(newNodeElement);

            let newNodeGroupViewingElement = TreePanelController.createNewNodeGroupViewingElement(panelsTrees[i], newNodeElement, index, radius);

            let newNodeGroupViewingElementVektor;

            if (i == 0 || i == 2) {
                newNodeGroupViewingElementVektor =
                    TreePanelController.createNewNodeGroupViewingElementVektor(newNodeElement, widthPanel, heightPanel, vertSpaceLocal, angleLocalLongestTrees, 6, 1, 1, 1);
            }
            else {

                newNodeGroupViewingElementVektor =
                    TreePanelController.createNewNodeGroupViewingElementVektor(newNodeElement, widthPanel, heightPanel, vertSpaceLocal, angleLocal, 6);
    ...
    for (let i = 0; i < valueArrOfArrays.length; i++) {
        ...
        for (let index = 0; index < someArray.length; index++) {
            ...
            if (i == 0 || i == 2) {
                slopeAngleLocal = angleLocalLongestTrees;
            }
            else {
                slopeAngleLocal = angleLocal;
            }

            if (newNodeElement.left != null && newNodeElement.right != null) {
                leftTempNode = newNodeElement.left;
                rightTempNode = newNodeElement.right;
                TreePanelController.drawArrow(actualPanel, newNodeElement, leftTempNode, radius);
                TreePanelController.drawArrow(actualPanel, newNodeElement, rightTempNode, radius);
            }
...
function drawDataInCloud() {
    ...
    let newTree = new BinarySearchTree(), posX, posY;

    for (let index = 0; index < labels.length; index++) {

        let newNodeElement = TreePanelController.createNewNodeElement(labels[index], newTree);
        let newNodeGroupViewingElement = TreePanelController.createNewNodeGroupViewingElement(panelCloud, newNodeElement, index, radius);
...
class QuestionsApplAnalysisSynthVisual extends Observable {
    ...
    constructor(el) {
        ...
        printCodePanel = new jsgl.Panel(el.querySelector("#print-code-panel"));
    ...
    getTreesSelectedInfo() {
        ...
        if (treesSelectionCheckboxesDiv.querySelector("#first-statement").checked) {
            wholeSkillsAsString += "Abbildung 2 zeigt möglichen Suchbaum für die zufällig eingefügten Schlüssel";
        }
        if (treesSelectionCheckboxesDiv.querySelector("#second-statement").checked) {
            wholeSkillsAsString += "Abbildungen 2 und 3 zeigen möglichen Suchbaum für die zufällig eingefügten Schlüssel";
        }
        if (treesSelectionCheckboxesDiv.querySelector("#third-statement").checked) {
            wholeSkillsAsString += "Alle Abbildungen zeigen möglichen Suchbaum für die zufällig eingefügten Schlüssel";
        }
        if (treesSelectionCheckboxesDiv.querySelector("#fourth-statement").checked) {
            wholeSkillsAsString += "Abbildungen 1 und 2 zeigen möglichen Suchbaum für die zufällig eingefügten Schlüssel";
        }
