...
function continueInnerAnimations(length) {
    ...
    if (positionOfMainCode == 22) {
        codeVisualizerViewing.designUnSelectedMainCodeGroup(positionOfMainCode, "viewing");
    }

    if (!(startAnimationIndex >= valueArray.length)) {
        if (innerIndex != (BST.getCurrentNode(valueArray[startAnimationIndex]).arrayOfVectors.length)) {
            innerAnimator = animatorsWholeTree[startAnimationIndex][innerIndex];
            currentPlayingAnimation = animatorsWholeTree[startAnimationIndex][innerIndex];
...
function prepareForAnimation() {
    for (let index = 0; index < valueArray.length; index++) {
        ...
        let nodeElement = TreePanelController.createNewNodeElement(valueArray[index], BST);
        nodeElements[index] = nodeElement;

        let nodeGroupViewingElement = TreePanelController.createNewNodeGroupViewingElement(panelViewing, nodeElement, index, radius);
        nodeGroupElements[index] = nodeGroupViewingElement;

        let nodeGroupViewingElementVektor = TreePanelController.createNewNodeGroupViewingElementVektor(nodeElement, widthOfPanel, heightOfPanel, verticalSpacing, a);
...
function animateMoving(arrayOfNodeVectors) {
        ...
        if (!(startAnimationIndex >= valueArray.length)) {
            ...
            if (startAnimationIndex != 0) {
                ...
                if (innerIndex != 0) {
                    ...
                    let newEqualityVector = new jsgl.Vector2D(precursorEndVector.getX(), precursorEndVector.getY() - 30);

                    if (endVector.getX() > precursorEndVector.getX()) { //nach rechts
                        groesserAls.setLocation(newEqualityVector);
                        addOnPanel(groesserAls);
                    }
                    else { //Knoten soll nach links gehen
            ...
            var time = Math.sqrt(tmp.direction.getX() * tmp.direction.getX() + tmp.direction.getY() * tmp.direction.getY()) / moveSpeed;
    ...
    innerAnimator.addStepListener(function (t) {
        if (!(startAnimationIndex >= valueArray.length)) {
            ...
            if (actualNodeGroupElements.length != 0) {
                actualNodeGroupElements[startAnimationIndex].setLocation(tmp.newLocation);
            }
        }
    });
    ...
    innerAnimator.addEndListener(function () {
        ...
        if ((innerIndex != (arrayOfVectors.length - 1)) && (actualNodeGroupElements.length != 0)) {
            innerIndex += 1;
            currentPlayingAnimation = animatorsWholeTree[startAnimationIndex][innerIndex];
            ...
            if (startAnimationIndex != 1 && startAnimationIndex != 0) {
                ...
                TreePanelController.drawArrow(panelViewing, parentOfAddedNode, addedNode, radius);
            }
            ...
            if (!(positionOfMainCode >= 23)) {
                codeVisualizerViewing.designUnSelectedMainCodeGroup(positionOfMainCode - 1, "viewing");
                codeVisualizerViewing.designSelectedMainCodeGroup(positionOfMainCode, "viewing");
            }
...
class ViewingVisualizer extends Observable {
    ...
    constructor(el) { //#viewing
        super();
        this.panelElementViewing = el.getElementsByClassName("panel viewing")[0];

        codeVisualizerViewing = new CodeVisualizer(el, "viewing");

        startButton = el.querySelector("#start-button");
        pauseButton = el.querySelector("#pause-button");
        ViewingVisualizer.setPauseButtonUnClickable();

        startButton.addEventListener("click", this.onViewingStartButtonClick.bind(this));
        pauseButton.addEventListener("click", this.onViewingPauseButtonClick.bind(this));
    ...
    onViewingPauseButtonClick(ev) {
        for (let index = 0; index < nodeGroupElementLocations.length; index++) {
            ...
            if (actualNodeGroupElements[startAnimationIndex].getLocation().getX() == element.getX()) {
                return;
            }
        }
